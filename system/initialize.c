...
struct	pipent	piptab[NPIPE];	/* Pipe array				*/

/* Active system status */

int	prcount;		/* Total number of live processes	*/
int 	pipcount;		/* number of pipe used			*/
...
static	void	sysinit(void)
{
	...
	struct	pipent	*pipptr;	/* ptr to pipe			*/
	...
	for (i = 0; i < NPIPE; i++) {
		...
		pipptr->reader = -1;
		pipptr->writer = -1;
		pipptr->emptySem = semcreate(0);
		pipptr->fullSem = semcreate(256);
		pipptr->head = 0;
		pipptr->tail = 0;
